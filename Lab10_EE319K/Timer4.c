...
void generateZombie (void) ; //zombie generate function
...
void Timer4_Init(void(*task)(void), uint32_t period){
  ...
  PeriodicTask4 = task;          // user function                                WHAT IS THIS
  ...
  NVIC_PRI5_R = (NVIC_PRI5_R&0xFFFFFF00)|0x00000000; // 8) priority 4            NEED TO CHANGE THE PRIORITY TO THE HIGHEST
